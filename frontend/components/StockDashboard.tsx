...
export default function StockDashboard({ simpleView = false }: { simpleView?: boolean }) {
    ...
    // In simpleView, just show top 4 movers (using default sort order which is usually interesting enough, or we could sort by volume/change)
    // For now, let's assume default list is okay, just take 4.
    ...
    // Trigger global search automatically if local is empty and query is valid (debounce could be better but let's do manual trigger or simple effect)
    // Lets use a manual "Enter" key or button for safety/rate-limiting, OR just show button if local is empty.

    return (
        <section id="dashboard" className="w-full max-w-7xl mx-auto px-6 py-10 scroll-mt-24">
                        ...
                        {displayData.map((stock, i) => (
                            <Link key={stock.symbol} href={`/stock/${stock.symbol}`}>
                                <motion.div
                                    ...
                                    className="glass-panel p-6 rounded-2xl hover:bg-white/10 transition-colors cursor-pointer group h-full border-t border-t-white/10"
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="text-lg font-bold group-hover:text-cyan-400 transition-colors flex items-center gap-1">
                                    ...
                                    <div className="flex flex-col gap-2">
                                        <div className="flex items-end justify-between">
                                            ...
                                            <span className={`text-sm font-bold px-2 py-0.5 rounded ${stock.status === 'up' ? 'text-green-400 bg-green-500/10' : 'text-red-400 bg-red-500/10'}`}>
